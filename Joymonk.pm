...
#>>>  A configuration hash reference
     my  $cf = {
     title	    => "Joymonk $VERSION a blissful place!",
     #site_image    => 'http://www.joymonk.com/images/flower.jpg',
     logo 	    => 'http://www.joymonk.com/images/flower.jpg',
     default_image  => 'http://www.joymonk.com/images/joymonk.jpg',
     url_perl       => '/admin',
     url_login      => '/perl/Login.pl',
     url_logout     => '/logout',
     url_google	    => 'http://www.google.com/search?hl=en&q=',
     admin_email    => 'bliss@joymonk.com',
     DEBUG          => 0,
     host           => '209.98.152.139',
     database       => 'joymonk',
     user           => 'postgres',
...
<link rel="shortcut icon" href="http://www.google.com/a/joymonk.com/images/logo.gif" type="image/x-icon" />
...
sub er_notice {
my ($msg, $r)  = @_ ;
system qq{ echo '$msg' | mail -s "Error Notice" ardeshir\@joymonk.com } ;
 
 if($? == -1) {
 $r->print( t_box("[ $msg ] please notify the system admin [ bliss at joymonk.com ] of this error!") ) ;
 } 
}
...
system qq( echo '$msg' | mail -s "$ag->{title}" $ag->{to} -c $ret->{email}  );
  ...
  if($? == -1) {
    $r->print( t_box("Sorry there was an error please notify the sys admin bliss [at] joymonk.com, thanks!") );
  } else {
...
if( $im ) {
my $image = qq{ <img src=http://www.joymonk.com$im->{url}> };
my $link  = qq{ http://www.joymonk.com$im->{url} };
...
if($_ eq 'image') {
   if( $ret->{image} eq "0") {	
    $ret->{image} = qq(<img src="http://www.joymonk.com/images/joymonk.jpg" alt=Smile width="$width"> );
   } else { 
...
if($_ eq 'image') {
   if( $ret->{image} eq "0") {	
    $ret->{image} = '<img src="http://www.joymonk.com/images/joymonk.jpg" valign=top align=left alt=Smile width=80>';
   } else { 
...
if($_ eq 'image') {
  if ($ret->{image} eq "0" ) {
   $ret->{image} = '<img src="http://www.joymonk.com/images/joymonk.jpg" align=left alt=Smile width=150>';
  } else { 
...
my $com;
   ...
   if($ag){
     ...
     if( ref($img_uid) ) {
       ...
       my $msg   = "\n"   . "Picture Comment Approval: joymonk.com" . "\n";  
       $msg      = "\n"   . "This is an automated response PLEASE don't *REPLY* to me, I'm just a friendly robot :-)" . "\n";   
       $msg     .= "\n\n" . "To view your picture comment click on this link:" . "\n";
       $msg     .= "\n"   . "http://www.joymonk.com$cf->{url_perl}?action=ck_comments" . "\n";
...
my $db_val = my_dbValue( "SELECT descr, user_id, url from images WHERE im_id=$ag->{pid}", my_dbh($cf), $r);
  ...
  if ( ref($db_val) ) {
   ...
   $r->print( t_box("Sorry, there was an error, please notify bliss [at] joymonk.com. Thanks!") );
...
sub my_blog {  
my ($ag, $cf, $ret, $r) = @_;
my $location = "http://www.joymonk.com/perl/blog.pl";     
...
sub my_images {  
my ($ag, $cf, $ret, $r) = @_;
my $location = "http://www.joymonk.com/perl/upload.pl";     
$r->headers_out->set(Location => $location);
$r->status(302);
return Apache::Const::REDIRECT;
}
...
sub my_music {  
my ($ag, $cf, $ret, $r) = @_;
my $location = "http://www.joymonk.com/perl/music.pl";     
$r->headers_out->set(Location => $location);
$r->status(302);
return Apache::Const::REDIRECT;
}
...
my ($ag, $cf )= @_;
return <<"BOX";
<div id="joyfriends"><a href="$cf->{url_perl}?action=joyfriends">Joymonk Friends</a><br/><div>
...
if($stmt =~ /^\s*select/i){
	...
	#$r->print( t_box( "There was an ERROR! Please email the admin at joymonk.com!" ) );
